  ...
  <div class="community-page">
    ...
    <main class="main-container">
      ...
      <div class="filter-panel">
        <el-radio-group v-model="sortType" class="elegant-radio" @change="fetchPosts">
          <el-radio-button label="latest">🌿 最新发布</el-radio-button>
          <el-radio-button label="trending">🔥 热门探讨</el-radio-button>
        </el-radio-group>
      </div>
      ...
      <div class="post-list" v-loading="loading">
        ...
        >
          <h3 class="post-title serif-text">
            {{ post.title }}
            <el-tag v-if="sortType === 'trending' && post.likes_count > 5" type="danger" size="small" effect="dark" round style="margin-left: 10px;">HOT</el-tag>
          </h3>
          ...
          <div class="post-meta">
            ...
            <div class="interaction-group">
              <span class="comments-count">
                💬 {{ post.comment_count }} 探讨
              </span>
              <span 
                class="like-btn" 
                :class="{ 'is-liked': post.is_liked > 0 }"
                @click.stop="toggleLike(post)"
              >
                {{ post.is_liked > 0 ? '❤️' : '🤍' }} {{ post.likes_count }}
              </span>
            </div>
          </div>
        </el-card>
...
const sortType = ref('latest') // 默认看最新
const dialogVisible = ref(false)
const submitting = ref(false)
const postForm = ref({ title: '', content: '' })
const currentUser = ref(null)

// 拉取帖子数据（携带排序类型和当前用户ID）
...
const toggleLike = async (post) => {
  if (!currentUser.value) return ElMessage.warning('请先登录后点赞')

  // 乐观更新：立刻让心变红，数字+1
  const originalLiked = post.is_liked > 0
  if (originalLiked) {
    post.is_liked = 0
    post.likes_count -= 1
  } else {
  ...
  try {
    ...
    if (!res.data.success) {
      throw new Error('点赞操作被服务端拒绝')
    }

    // ====== 核心升级：热门榜单下的动态打榜重排 ======
    // 只有在“热门”板块点赞，才触发重新排序
    if (sortType.value === 'trending') {
      setTimeout(() => {
        // 使用与后端完全一致的权重算法计算分数
        postList.value.sort((a, b) => {
          const scoreA = a.likes_count * 2 + a.comment_count * 5
          const scoreB = b.likes_count * 2 + b.comment_count * 5
          return scoreB - scoreA // 分数高的排在前面
        })
      }, 500) // 延迟 500 毫秒，让用户看清红心动画后再提升排名
    }

  } catch (error) {
    // 接口报错时，状态回退
    post.is_liked = originalLiked ? 1 : 0
    post.likes_count = originalLiked ? post.likes_count + 1 : post.likes_count - 1
...
const submitPost = async () => {
  ...
  try {
    ...
    if (res.data.success) {
      ...
      sortType.value = 'latest' // 发帖后自动切回最新板块
      fetchPosts() 
    }
  } catch (error) {
...
/* 榜单切换组件美化 */
.filter-panel { margin-bottom: 25px; display: flex; justify-content: flex-start; }
:deep(.elegant-radio .el-radio-button__inner) { background: transparent; border: 1px solid #c8dcd0; color: #8fa799; font-weight: bold; box-shadow: none !important; }
:deep(.elegant-radio .el-radio-button:first-child .el-radio-button__inner) { border-radius: 20px 0 0 20px; }
:deep(.elegant-radio .el-radio-button:last-child .el-radio-button__inner) { border-radius: 0 20px 20px 0; }
:deep(.elegant-radio .el-radio-button.is-active .el-radio-button__inner) { background: var(--primary-green); border-color: var(--primary-green); color: white; }
...
.post-title { margin: 0 0 10px 0; font-size: 20px; color: var(--deep-green); display: flex; align-items: center;}
...
/* 点赞与评论交互区域 */
.interaction-group { display: flex; gap: 15px; align-items: center; }
.like-btn { cursor: pointer; padding: 4px 8px; border-radius: 12px; transition: all 0.2s; font-weight: bold; color: #a4babb; background: #f0f5f2; user-select: none;}
.like-btn:hover { background: #e0ede5; transform: scale(1.05); }
.like-btn.is-liked { color: #e74c3c; background: #fdf0f0; }
.like-btn.is-liked:hover { background: #fce4e4; }
